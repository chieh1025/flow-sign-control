...
const nodeTypeStyles: Record<string, string> = {
  start: "rounded-full border-green-500 bg-green-50",
  end: "rounded-full border-orange-500 bg-orange-50",
  task: "rounded-lg border-blue-500 bg-white",
  decision: "rounded-lg border-purple-500 bg-purple-50 rotate-0",
  connector: "rounded-full border-amber-500 bg-amber-50",
};

const signMethodLabel: Record<string, { text: string; color: string }> = {
  system_sign: { text: "系統簽", color: "bg-green-100 text-green-700" },
  paper_sign: { text: "紙本簽", color: "bg-orange-100 text-orange-700" },
  both: { text: "系統+紙本", color: "bg-blue-100 text-blue-700" },
};
...
function ProcessNodeComponent({ id, data, selected }: NodeProps) {
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "border-2 px-4 py-3 min-w-[200px] max-w-[280px] shadow-sm transition-all",
          style,
      ...
      >
        ...
        <div className="flex flex-wrap gap-1 mb-1.5">
          {nodeData.operatingSystem && (
            <StatusBadge label={nodeData.operatingSystem} color="bg-blue-100 text-blue-700" />
          )}
          ...
          {nodeData.status.vacant && (
            <StatusBadge label="缺人" color="bg-red-100 text-red-700" />
          )}
          {nodeData.status.unsigned && (
            <StatusBadge label="未簽" color="bg-yellow-100 text-yellow-700" />
          )}
          {nodeData.status.paperSign && (
            <StatusBadge label="紙本簽" color="bg-gray-100 text-gray-600" />
          )}
          {nodeData.status.other && (
            <StatusBadge label={nodeData.status.other} color="bg-gray-100 text-gray-600" />
          )}
        </div>

        {/* Node name */}
        <div className="font-semibold text-base text-gray-800">{nodeData.label}</div>
