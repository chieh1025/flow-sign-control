  ...
  return (
    <>
      ...
      <div className={cn(
        ...
        "border-l-4", stripe,
        // Other borders
        "border-t border-r border-b border-slate-100 dark:border-slate-600",
        // Shadow elevation
        ...
        <div className="px-4 py-2.5">
          <div className="font-medium text-[13px] text-slate-900 dark:text-white">{d.label}</div>
          {d.holder && (
            <div className="text-[11px] text-slate-500 dark:text-slate-300 mt-0.5">{d.holder}</div>
          )}
          {d.title && (
            <div className="text-[11px] text-slate-400 dark:text-slate-400 mt-0.5">{d.title}</div>
          )}
        </div>
      </div>
